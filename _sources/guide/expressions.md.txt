...
error. Quoting also lets an isotope name that contains a `#` be written without
tripping YAML's comment rule:
...
```julia
pj.evaluate_pals_expression("3.75e7 / c_light^2")   # 4.172…e-10
pj.evaluate_pals_expression("mass_of(\"electron\")") # 510998.95069…
pj.evaluate_pals_expression("expr(2 * pi)")          # 6.283…
```
